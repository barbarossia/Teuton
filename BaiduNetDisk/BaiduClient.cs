...
namespace BaiduNetDisk
{
    public class BaiduClient
    {
        ...
        public string UploadSlice(string sourcePath)
        {
            var client = new RestClient(BaseUrl);
            var request = new RestRequest("file?method={method}&access_token={access_token}&type={type}", Method.POST);
            request.AddParameter("method", "upload", ParameterType.UrlSegment);
            request.AddParameter("access_token", AccessToken, ParameterType.UrlSegment);
            request.AddParameter("type", "tmpfile", ParameterType.UrlSegment);
            request.Timeout = int.MaxValue;

            request.AddFile(Path.GetFileName(sourcePath), sourcePath);
            request.AlwaysMultipartFormData = true;

            var response = client.Execute(request);
            HandleResponseExcepition(response);
            return response.Content.ToMD5();
        }

        public NetFileInfo UploadSuperFile(string destPath, string[] blockList)
        {
            var client = new RestClient(BaseUrl);
            var request = new RestRequest("file?method={method}&path={path}&access_token={access_token}", Method.POST);
            request.AddParameter("method", "createsuperfile", ParameterType.UrlSegment);
            request.AddParameter("access_token", AccessToken, ParameterType.UrlSegment);
            request.AddParameter("path", destPath, ParameterType.UrlSegment);
            request.AddParameter("param", CreateBlockList(blockList));
        ...
        private string CreateBlockList(string[] blockList)
        {
            return blockList.Aggregate("{\"block_list\":[\"", (output, item) => string.Format("{0}{1}\",\"", output, item), (output) => output.Substring(0, output.Length - 2) + "]}");
        }
    }
}
